...
<script language="javascript" type="text/javascript">
	...
	function goDelete(id,f){
		if (confirm('are you sure to delete?')!=0){
			...
			var status = AjaxRequest.submit(
				...
				,{
				  'url':'?m=tstt&op=ban'
				  ,'onSuccess':function(req){ document.getElementById('tsttListban').innerHTML=req.responseText;}
	...
	function callSearch(f){
		var status = AjaxRequest.submit(
			...
			,{
			  'url':'?m=tstt'
			  ,'onSuccess':function(req){ document.getElementById('tsttList').innerHTML=req.responseText;}
			  ,'onError':function(req){}
			}
		  );
		  progress('tsttListban');
		  return status;		
	}
	//
...
{/literal}
<div class="topic">Quản lý hợp đồng bán tstt</div>
<div align="right"><a href="?m=tstt&op=frm" class="title">{$Create}</a></div>
<div id="tsttList">{tsttListban}</div>
