...
<script language="javascript" type="text/javascript">
	...
	function goDelete(id,f){
		if (confirm('are you sure to delete?')!=0){
			...
			var status = AjaxRequest.submit(
				...
				,{
				  'url':'?m=tsbv&op=ban'
				  ,'onSuccess':function(req){ document.getElementById('tsbvListban').innerHTML=req.responseText;}
	...
	function callSearch(f){
		var status = AjaxRequest.submit(
			...
			,{
			  'url':'?m=tsbv'
			  ,'onSuccess':function(req){ document.getElementById('tsbvList').innerHTML=req.responseText;}
			  ,'onError':function(req){}
			}
		  );
		  progress('tsbvListban');
		  return status;		
	}
	//
...
<div align="right"><a href="?m=tsbv&op=frm" class="title">{$Create}</a></div>
<div id="tsbvList">{tsbvListban}</div>
