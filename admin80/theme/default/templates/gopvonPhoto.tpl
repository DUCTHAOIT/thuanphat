	...
	function goDelete(id,f){
		...
		var status = AjaxRequest.submit(
			...
			,{
			  ...
			  ,'onSuccess':function(req){ document.getElementById('gopvonListPhoto').innerHTML=req.responseText;}
			  ,'onError':function(req){}
			}
		  );		  
		  progress('gopvonListPhoto');
		  return status;	
		}
	}
...
{/literal}
<form name="frmmain" action="?m=gopvon" method="post" enctype="multipart/form-data" id="formUpload" onSubmit="return false;">
<input type="hidden" name="op" value="addPhoto" />
<input type="hidden" name="id" value="{$id}" />
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td colspan="2" style="padding-bottom:15px;"><strong>{$Management_gopvons} {$arr.name}</strong></td>
    </tr>
  <tr>
    <td align="center">
		{if $arr.img}
	<img src="{$smarty.const._DOMAIN_ROOT_URL}/images/gopvon/{$arr.img}"  width="250px"/>
	{else}
		Not image
	{/if}</td>
...
<div id="gopvonListPhoto">{gopvonListPhoto}</div>
{literal}
