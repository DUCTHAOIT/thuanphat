...
<div style="text-align:right; padding-bottom:10px"><a href="?m=nhanvien&op=frmCreate" class="title">{$nhanvien_create}</a></div>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" align="right"> Lựa chọn: </td>
    <td nowrap="nowrap" class="title" >Năm: &nbsp;</td>
    <td align="right" nowrap="nowrap">			
    <select name="year" onchange="location = '{$smarty.const._DOMAIN_ROOT_URL}/admin80/?m=nhanvien&op=frmCreate{if $id}&id={$id}{/if}{if $month}&month={$month}{/if}&year='+ this.options[this.selectedIndex].value;" style="font-size:12px" >
    ...
    <td align="right" nowrap="nowrap">			
    <select name="month" onchange="location = '{$smarty.const._DOMAIN_ROOT_URL}/admin80/?m=nhanvien&op=frmCreate{if $id}&id={$id}{/if}{if $year}&year={$year}{/if}&month='+ this.options[this.selectedIndex].value;" style="font-size:12px" >
...
{foreach key=key item=item from=$arrnhanvienHD}
...
{if 500000000 < $total && $total < 2000000000}{assign var="hoahong" value="`$total*0.0015`"}{/if}
{if 2000000000 < $total && $total < 5000000000}{assign var="hoahong" value="`$total*0.003`"}{/if}
{if 5000000000 < $total && $total < 10000000000}{assign var="hoahong" value="`$total*0.0045`"}{/if}
{if 10000000000 < $total}{assign var="hoahong" value="`$total*0.006`"}{/if}
