...
<div><h2>Khách hàng giới thiệu</h2></div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-striped table-bordered dataTable">
  <tr style="background-color:#0b7a62">
    ...
    <td  style="vertical-align: middle; text-align:center; color:#FFFFFF; font-size:0.8rem;"><strong>Ngày đăng ký</strong></td>
  ...
  {foreach key=key item=item from=$arrUserGioithieu}
      <tr bgcolor="{cycle values="#ffffff,#F4F4F4"}">
        ...
        <td style="text-align: justify; vertical-align: middle;"><a href="{$smarty.const._DOMAIN_ROOT_URL}/{$item.url}{$item.alias}"  target="_blank">{$item.nameproduct}</a></td>
   ...
   {/foreach}
      <tr>
        <td colspan="6" class="title">Tổng cộng:</td>
