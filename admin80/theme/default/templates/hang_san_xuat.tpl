{literal}
	<script language="javascript">
		...
		function searchList(str)
		{
			AjaxRequest.get(
			{
			'url':'?m=product&f=manufacturers&op=search&search='+ str
			,'onSuccess':function(req){document.getElementById('td_manufacturersList').innerHTML=req.responseText;}
...
<form name="frmmain" action="?m=product&f=manufacturers" method="post" enctype="multipart/form-data">
  ...
  <tr>
    <td width="50%" valign="top" style="padding:10px; border:1px solid #D3D7DC">
      ...
      <tr>
		...
		<label onclick="WindowUpload('logo')" style="cursor:pointer" class="title">Upload Image</label></td>
      </tr>
    ...
    <td valign="top" style="padding-left:10px" id="td_manufacturersList">{manufacturersList}</td>
  </tr>
</table>
</form>