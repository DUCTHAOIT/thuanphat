	...
	function goDelete(id,f){
		if (confirm('are you sure to delete?')!=0){
			...
			var status = AjaxRequest.submit(
				f
				,{
				  'url':'?m=hdmuachung&op=ban'
				  ,'onSuccess':function(req){ document.getElementById('hdmuachungListban').innerHTML=req.responseText;}
...
<div id="hdmuachungList">{hdmuachungListban}</div>
