...
<script language="javascript" type="text/javascript">
	...
	function goDelete(id,f){
		if (confirm('are you sure to delete?')!=0){
			...
			var status = AjaxRequest.submit(
				...
				,{
				  'url':'?m=userorder'
				  ,'onSuccess':function(req){ document.getElementById('userorderList').innerHTML=req.responseText;}
	...
	function callLock(id){
		AjaxRequest.get(
			{
			'url':'?m=userorder&op=lockuserorder&id='+id
...
<table border="0" cellspacing="0" cellpadding="0">
  ...
  <tr bgcolor="#E3E6EB" height="30">
    <td style="padding-left:10px" nowrap="nowrap"><strong>{$userorder_group}:</strong></td>
    <td style="padding-left:10px">
	<select name="catID" style="border:1px solid #cccccc;">
	<option value="" selected="selected"></option>
	{foreach key=key item=item from=$arrTopicuserorder}
...
<div align="right"><a href="?m=userorder&op=frm" class="title">{$Create}</a></div>
<div id="userorderList">{userorderList}</div>
