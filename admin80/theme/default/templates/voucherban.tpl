...
<script language="javascript" type="text/javascript">
	...
	function goDelete(id,f){
		if (confirm('are you sure to delete?')!=0){
			...
			var status = AjaxRequest.submit(
				...
				,{
				  'url':'?m=topup&op=ban'
				  ,'onSuccess':function(req){ document.getElementById('topupListban').innerHTML=req.responseText;}
	...
	function callSearch(f){
		var status = AjaxRequest.submit(
			...
			,{
			  'url':'?m=topup'
			  ,'onSuccess':function(req){ document.getElementById('topupList').innerHTML=req.responseText;}
			  ,'onError':function(req){}
			}
		  );
		  progress('topupListban');
		  return status;		
	}
	//
	...
	function callLock(id){
		AjaxRequest.get(
			{
			'url':'?m=topup&op=locktopup&id='+id
...
<div align="right"><a href="?m=topup&op=frm" class="title">{$Create}</a></div>
<div id="topupList">{topupListban}</div>
