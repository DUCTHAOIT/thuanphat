...
<table width="100%" border="1" cellspacing="2" cellpadding="2" class="table" style="border:1px solid #CCCCCC">
  ...
  <tr  {if $item.hoancoc=='1'}bgcolor="#FFFF00"{else}bgcolor="{cycle values="#ffffff,#F4F4F4"}"{/if}>
   <td align="left" class="td" nowrap="nowrap">{$key}</td>
    ...
    <td class="td" nowrap="nowrap" align="center"><a href="?m=hdmuachung&userid={$item.userid}" class="title" style="text-transform:uppercase">{$item.name}</a><br />{$item.email}<br />{$item.mobile}<br />(<a href="?m=user&op=frmCreate&id={$item.email}" class="content"><i>Thông tin</i></a>)</td>
  ...
  <tr>
    	...
    	<td class="title" colspan="7" align="right">Tổng cộng</td>
