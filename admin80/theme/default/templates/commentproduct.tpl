...
function goDelete(id,f){
	if (confirm('are you sure to delete?')!=0){
		...
		progress('commentproductList');	
		var status = AjaxRequest.submit(
			f
			,{
			  'url':window.location.search
			  ,'onSuccess':function(req){ document.getElementById('commentproductList').innerHTML=req.responseText;}
			  ,'onError':function(req){}
			}
		  );		  
		  return status;	
...
{/literal}
<div style="text-align:right; padding-bottom:5px"><a href="?m=commentproduct&op=frm" class="title">{$Create}</a></div>
<div id="commentproductList">{commentproductList}</div>
