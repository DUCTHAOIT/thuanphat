{literal}{/literal}
<div style="text-align:right"><strong>{$countArr}</strong> {$gopvons_in_the_list}</div>
...
<table width="100%%" border="1" cellspacing="0" cellpadding="0" class="table">
  ...
  <tr bgcolor="{cycle values="#ffffff,#F4F4F4"}">
   <td align="center" class="td">{$key}</td>	
    <td class="td">
	{if $item.img}
		<img src="{$smarty.const._DOMAIN_ROOT_URL}/images/gopvon/{$item.img}" width="70" onclick="goEdit({$key})" style="cursor:pointer" />
	{else}
    ...
    Giá: {format_number number=$item.price}đ<br />
