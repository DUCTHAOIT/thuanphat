...
//		$alias= $obj->remoteDiacritic($name);	
		...
		if(file_exists($sourcefile)){			
			$from=$sourcefile;
			$to=_DOMAIN_ROOT_PATH."/images/tstt/".$imgsmall;
			moveFile($from,$to);			
		}
		$sourcefile=_DOMAIN_ROOT_PATH."/temp/".$imgbig;		
		if(file_exists($sourcefile)){		
			$from=_DOMAIN_ROOT_PATH."/temp/".$imgbig;
			$to=_DOMAIN_ROOT_PATH."/images/tstt/".$imgbig;
			moveFile($from,$to);
	...
	//
	function gettsttList_($all,$pageID,$limit=20){
		global $db;
	...
	//
	function gettsttList($all,$pageID,$limit=20){
		global $db;
		loadClass("constructSql");		
		$obj=new constructSql();
		...
		$obj->where="sys_userorder.loai=1 AND sys_userorder.hdban=0 AND user.id=sys_userorder.userid";	
	...
	//
	//
	function gettsttListBan($all,$pageID,$limit=20){
		global $db;
		loadClass("constructSql");		
		$obj=new constructSql();
		$userid=getParam("userid");
	...
	function gettsttID($proID){
		...
		else $arr["imgs_view"]=_DOMAIN_ROOT_URL."/images/tstt/".$arr["img"];
		if(!$arr["img1"]) $arr["imgb_view"]=_DOMAIN_ROOT_URL."/images/none.gif";
		else $arr["imgb_view"]=_DOMAIN_ROOT_URL."/images/tstt/".$arr["img1"];
		return $arr;
	}
	//
	//
	function deleteban(){
		...
		include(_DOMAIN_ROOT_PATH . "/classes/msgbox.class.php");
		$ret_page="/admin80/?m=tstt&f=hdban";
	...
	function locktstt(){
		...
		if($rs->fields("ctrl")=='1' && $rs->fields("model")>0){
			...
			$sql="SELECT sys_userorder.*, DATE_FORMAT(sys_userorder.date_create, '%d/%m/%Y') as date_create, user.email email, user.name username FROM sys_userorder,user WHERE sys_userorder.loai=1 AND sys_userorder.id=$id AND user.id=sys_userorder.userid";
			$rs=$db->Execute($sql);
	...
	function daban(){
		...
		if($conlai>0){
			//$sqlhdban="UPDATE sys_userorder SET product_in='0' WHERE id=$hdban";	
			$sqlhdban="UPDATE sys_userorder SET product_in='0', price_old='$price_old' WHERE id=$hdban";	
			$db->Execute($sqlhdban);
		}else{
			$sqlhdban="UPDATE sys_userorder SET trangthai=IF(trangthai=0,1,0), product_in='0' WHERE id=$hdban";		
			$db->Execute($sqlhdban);
		}
	...
	function addPhoto(){
		...
		foreach($_FILES['img_file']['name'] as $name => $value)
		{
			$name_img = stripslashes(date('YmdHis')."_".$_FILES['img_file']['name'][$name]);
			$source_img = $_FILES['img_file']['tmp_name'][$name];
			$path_img = _DOMAIN_ROOT_PATH."/images/tstt/".$name_img;	
		...
		include(_DOMAIN_ROOT_PATH . "/classes/msgbox.class.php");		
		if(!$return){
			$ret_page="?m=tstt&op=photo&id=$id";
			$a=new msgBox(_CANNOT_UPDATE_SUCCESSFU,"OKOnly", "Message", array($ret_page), 1);			
			$a->showMsg();
		}else{
			$ret_page="?m=tstt&op=photo&id=$id";
	...
	//
	function gettsttListPhoto($proID){
		global $db;		
	...
	function addFile(){
		...
		include(_DOMAIN_ROOT_PATH . "/classes/msgbox.class.php");
		$ret_page="?m=tstt&op=file&id=$id";
		$a=new msgBox(_UPDATE_SUCCESSFU,"OKOnly", "Message", array($ret_page), 1);			
		$a->showMsg();
	}
	//
	//
	function gettsttListFile($proID){
		global $db;		
