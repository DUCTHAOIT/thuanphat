...
include_once("header.php");
	...
	$rs=$db->Execute($sql);	
	if($rs->RecordCount()){	?>	
 <div class="topic">Quản lý hợp đồng bán TSTT <button><a href="http://thachsanhinvestment.com.vn/admin80/?m=tsbv&f=01simple-download-xlsx" target="_blank">Download dữ liệu</a></button></div> 
...
?>
  ...
  <tr <?php if($j%2){?> bgcolor="#F4F4F4"<?php }?>>
    ...
    <td class="td" align="right"><?php if($giadvdtchot>0){ echo number_format($tncn, 0, '.', ',')."";}else{ echo "#";} ?></td>
     ...
     <td class="td" align="right"><?php if($giadvdtchot>0){ echo number_format($hoahongck*0.9, 0, '.', ',')."";}else{ echo "#";} ?></td>
  	...
  	<?php if($rs->fields("price")){?>
        ...
        <td align="center" class="td"><?php if($rs->fields("trangthai")==1){?><img src="images/daban<?php echo $rs->fields("trangthai");?>.gif" style="cursor:pointer" alt="Đã chuyển khoản" /><?php }else{?><a href="<?php echo _DOMAIN_ROOT_URL;?>/admin80/?m=tsbv&op=daban&id=<?php echo $rs->fields("id");?>&hdban=<?php echo $rs->fields("hdban");?>&soluongban=<?php echo $rs->fields("model");?>" onclick = "if (! confirm('Bạn muốn xác nhận giao dịch đã hoàn thành chuyển khoản cho NĐT?')) { return false; }" title="click để xác nhận chuyển khoản" class="title" style="font-size:12px; color:#FFFFFF"><img src="images/daban<?php echo $rs->fields("trangthai");?>.gif" style="cursor:pointer" alt="click để xác nhận chuyển khoản" /></a><?php }?></td>
	<?php }else{?>
    <td class="td"  align="center" >
     ...
     <td align="center" class="td"><?php if($rs->fields("trangthai")==2){?><img src="images/huy<?php echo $rs->fields("trangthai");?>.gif" style="cursor:pointer" alt="click để hủy giao dịch" /><?php }else{?><a href="<?php echo _DOMAIN_ROOT_URL;?>/admin80/?m=tsbv&op=huy&id=<?php echo $rs->fields("id");?>&soluongban=<?php echo $rs->fields("model");?>" onclick = "if (! confirm('Bạn muốn hủy giao dịch?')) { return false; }" title="click để hủy giao dịch" class="title" style="font-size:12px; color:#FFFFFF"><img src="images/huy<?php echo $rs->fields("trangthai");?>.gif" style="cursor:pointer" alt="click để hủy giao dịch" /></a><?php }?></td>
    <?php }?>
    
    
   <!-- 
    <td align="center" class="td"><a href="<?php echo _DOMAIN_ROOT_URL;?>/admin80/?m=tsbv&op=deleteban&id=<?php echo $rs->fields("id");?>" onclick = "if (! confirm('Bạn muốn hủy giao dịch?')) { return false; }" class="title" style="font-size:12px; color:#FFFFFF"><img src="images/delete.gif" style="cursor:pointer" /></a></td>
    -->
  </tr>	
