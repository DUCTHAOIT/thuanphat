<?php	
	function getTopicgopvon($proID=""){
		global $db,$moduleName;
		$sql="SELECT * FROM sys_function WHERE (lang='vn') AND module='gopvon' AND (ctrl&1=1) ORDER BY id";	
		$arr=$db->getAssoc($sql);					
		return $arr;
	}
	function getTopicgopvon_($proID=""){
		global $db,$moduleName;
		loadClass("menuLevel");				
...
//		$alias= $obj->remoteDiacritic($name);	
		...
		if(file_exists($sourcefile)){			
			$from=$sourcefile;
			$to=_DOMAIN_ROOT_PATH."/images/gopvon/".$imgsmall;
			moveFile($from,$to);			
		}
		$sourcefile=_DOMAIN_ROOT_PATH."/temp/".$imgbig;		
		if(file_exists($sourcefile)){		
			$from=_DOMAIN_ROOT_PATH."/temp/".$imgbig;
			$to=_DOMAIN_ROOT_PATH."/images/gopvon/".$imgbig;
			moveFile($from,$to);
		...
		$soxuatdautu1=str_replace($vowels, "", getParamPost("soxuatdautu1"));
		...
		$codongtoida=str_replace($vowels, "", getParamPost("codongtoida"));
		...
		if(!$id){			
			$sql = "SELECT id FROM sys_gopvon WHERE alias = '$alias'";			
			$rs = $db->Execute($sql);
			if(!$rs->RecordCount()){
					
					$sql = "SELECT * FROM sys_gopvon WHERE 0 = -1";
					$rs = $db->Execute($sql);
					//$sql = $db->GetInsertSQL($rs, $record);			
					$sql = "INSERT INTO sys_gopvon (catID, name, alias, summary, content, model, delivery, product_in, promotion, position, solutions, tienich, vitri, price_old, price, tinh_trang, img, img1, pdf, date_create, lang, special_promotion, sort,giaidoan,xuatsu,manufacturers,chinhsach, title, description, keywords, tengiaidoan1,hinhthuc1,tongvondautu1,soxuatdautu1,sotienmotxuat1,soxuatdakeugoi1,tinhtrangduan1,gioithieu1,tiendo1,phaply1,tengiaidoan2,hinhthuc2,tongvondautu2,soxuatdautu2,sotienmotxuat2,soxuatdakeugoi2,tinhtrangduan2,gioithieu2,tiendo2,phaply2,tengiaidoan3,hinhthuc3,tongvondautu3,soxuatdautu3,sotienmotxuat3,soxuatdakeugoi3,tinhtrangduan3,gioithieu3,tiendo3,phaply3,chietkhau1,chietkhau12,chietkhau2,chietkhau22,chietkhau3,chietkhau32,sotientoithieu,sotientoida,codongtoida) VALUES ( '$catID', '$name', '$alias', '$summary', '$content', '$model',  '$delivery', '$product_in', '$promotion', '$position', '$solutions', '$tienich', '$vitri', '$price_old', '$price_new', '$tinh_trang', '$imgsmall', '$imgbig', '$filePDF', '$date', '$langID', '$special_promotion', '$sort','$giaidoan','$str_xuatsu','$str_manufacturers','$chinhsach', '$title','$description','$keywords','$tengiaidoan1','$hinhthuc1','$tongvondautu1','$soxuatdautu1','$sotienmotxuat1','$soxuatdakeugoi1','$tinhtrangduan1','$gioithieu1','$tiendo1','$phaply1','$tengiaidoan2','$hinhthuc2','$tongvondautu2','$soxuatdautu2','$sotienmotxuat2','$soxuatdakeugoi2','$tinhtrangduan2','$gioithieu2','$tiendo2','$phaply2','$tengiaidoan3','$hinhthuc3','$tongvondautu3','$soxuatdautu3','$sotienmotxuat3','$soxuatdakeugoi3','$tinhtrangduan3','$gioithieu3','$tiendo3','$phaply3','$chietkhau1','$chietkhau12','$chietkhau2','$chietkhau22','$chietkhau3','$chietkhau32','$sotientoithieu','$sotientoida','$codongtoida')";			
					$return=$db->Execute($sql);	
					if($return){
						$idNew=$db->Insert_ID();
						foreach($groupID as $key=>$value){
							$sql="INSERT INTO sys_gopvon_admin_cat(catID,artID) VALUES('$value','$idNew')";
							$return=$db->Execute($sql);
						}
					}						
			}else{
			...
			if(!$rs->RecordCount()){
					...
					$rs = $db->Execute($sql);						
					//$sql = $db->GetUpdateSQL($rs, $record);
					//$sql= "UPDATE sys_gopvon SET catID = '$catID', name = '$name', alias = '".$alias."-".$id."', summary = '$summary', content = '$content', model = '$model', delivery='$delivery', gopvon_in='$gopvon_in', promotion='$promotion', position='$position', solutions='$solutions', tienich='$tienich', price_old = '$price_old', price = '$price_new', tinh_trang = '$tinh_trang', img = '$imgsmall', img1 = '$imgbig', pdf = '$filePDF', date_create = '$date', lang = '$langID', special_promotion = '$special_promotion' WHERE id=$id";
					$sql= "UPDATE sys_gopvon SET catID = '$catID', name = '$name', alias = '".$alias."', summary = '$summary', content = '$content', model = '$model', delivery='$delivery', product_in='$product_in', promotion='$promotion', position='$position', solutions='$solutions', tienich='$tienich', vitri='$vitri', price_old = '$price_old', price = '$price_new', tinh_trang = '$tinh_trang', img = '$imgsmall', img1 = '$imgbig', pdf = '$filePDF', date_create = '$date', lang = '$langID', special_promotion = '$special_promotion', sort = '$sort',giaidoan='$giaidoan',xuatsu='$str_xuatsu',manufacturers='$str_manufacturers',chinhsach='$chinhsach',title='$title',description='$description',keywords='$keywords',tengiaidoan1='$tengiaidoan1',hinhthuc1='$hinhthuc1',tongvondautu1='$tongvondautu1',soxuatdautu1='$soxuatdautu1',sotienmotxuat1='$sotienmotxuat1',soxuatdakeugoi1='$soxuatdakeugoi1',tinhtrangduan1='$tinhtrangduan1',gioithieu1='$gioithieu1',tiendo1='$tiendo1',phaply1='$phaply1',tengiaidoan2='$tengiaidoan2',hinhthuc2='$hinhthuc2',tongvondautu2='$tongvondautu2',soxuatdautu2='$soxuatdautu2',sotienmotxuat2='$sotienmotxuat2',soxuatdakeugoi2='$soxuatdakeugoi2',tinhtrangduan2='$tinhtrangduan2',gioithieu2='$gioithieu2',tiendo2='$tiendo2',phaply2='$phaply2',tengiaidoan3='$tengiaidoan3',hinhthuc3='$hinhthuc3',tongvondautu3='$tongvondautu3',soxuatdautu3='$soxuatdautu3',sotienmotxuat3='$sotienmotxuat3',soxuatdakeugoi3='$soxuatdakeugoi3',tinhtrangduan3='$tinhtrangduan3',gioithieu3='$gioithieu3',tiendo3='$tiendo3',phaply3='$phaply3',chietkhau1='$chietkhau1',chietkhau12='$chietkhau12',chietkhau2='$chietkhau2',chietkhau22='$chietkhau22',chietkhau3='$chietkhau3',chietkhau32='$chietkhau32',sotientoithieu='$sotientoithieu',sotientoida='$sotientoida',codongtoida='$codongtoida' WHERE id=$id";
					$return=$db->Execute($sql);
	...
	//
	function getgopvonList($all,$pageID,$limit=20){
		global $db;
		loadClass("constructSql");		
		$obj=new constructSql();
	...
	//	echo "duong".$hang_san_xuat;		
		if(!$hang_san_xuat){ 			
			if(!$keyword){
				$obj->fieldsName="sys_gopvon.*, DATE_FORMAT(sys_gopvon.date_create, '".format_date()."') as date_create, TO_DAYS(sys_gopvon.date_create) as today, sys_function.name as nameCat";
				$obj->tableName="sys_gopvon ,sys_function";		
						
				if(!$catID){ $obj->where="(sys_gopvon.catID =  sys_function.id)";}
				else {$obj->where="(sys_gopvon.catID=$catID) AND (sys_gopvon.catID =  sys_function.id)";}						
				$obj->orderBy="today DESC";						
			}else{		
				$obj->fieldsName="sys_gopvon.*, DATE_FORMAT(sys_gopvon.date_create, '".format_date()."') as date_create, TO_DAYS(sys_gopvon.date_create) as today, sys_function.name as nameCat, match(sys_gopvon.name,sys_gopvon.model) against('$keyword' in boolean mode) as relevance";
				$obj->tableName="sys_gopvon ,sys_function";
				
				if(!$catID) $obj->where="(sys_gopvon.catID =  sys_function.id) AND (match(sys_gopvon.name, sys_gopvon.model) against('$keyword' in boolean mode))";		
				else $obj->where="(sys_gopvon.catID=$catID) AND (sys_gopvon.catID =  sys_function.id) AND (match(sys_gopvon.name, sys_gopvon.model) against('$keyword' in boolean mode))";		 			
				
				$obj->orderBy="relevance DESC";				
			}
		}else{		
			if(!$keyword){
					$obj->fieldsName="sys_gopvon.*, DATE_FORMAT(sys_gopvon.date_create, '".format_date()."') as date_create, TO_DAYS(sys_gopvon.date_create) as today, sys_function.name as nameCat, hang_san_xuat.name as nameHSX";
					$obj->tableName="sys_gopvon,sys_function,hang_san_xuat";		
					
					$obj->where="(sys_gopvon.hang_san_xuat=$hang_san_xuat) AND (sys_gopvon.hang_san_xuat =  hang_san_xuat.id) AND";
					...
					$obj->fieldsName="sys_gopvon.*, DATE_FORMAT(sys_gopvon.date_create, '".format_date()."') as date_create, TO_DAYS(sys_gopvon.date_create) as today, sys_function.name as nameCat, hang_san_xuat.name as nameHSX, match(sys_gopvon.name,sys_gopvon.model) against('$keyword' in boolean mode) as relevance";
					$obj->tableName="sys_gopvon,sys_function,hang_san_xuat";
	...
	//
	function getgopvonID($proID){
		global $db;
		loadClass("constructSql");
		
		$obj=new constructSql();
		$obj->fieldsName="*, DATE_FORMAT(date_create, '%Y-%m-%d') as date_create";
		$obj->tableName="sys_gopvon";		
		$obj->where="id=$proID";
		$sql=$obj->sqlSelect();
		$rs=$db->Execute($sql);
		$arr=$rs->fields;
		if(!$arr["img"]) $arr["imgs_view"]=_DOMAIN_ROOT_URL."/images/none.gif";
		else $arr["imgs_view"]=_DOMAIN_ROOT_URL."/images/gopvon/".$arr["img"];
		if(!$arr["img1"]) $arr["imgb_view"]=_DOMAIN_ROOT_URL."/images/none.gif";
		else $arr["imgb_view"]=_DOMAIN_ROOT_URL."/images/gopvon/".$arr["img1"];
		return $arr;
	}
	
	//
	function deletegopvon(){
		global $db,$lable;
		...
		if(($rs->fields("img")) or ($rs->fields("img1"))){
			...
			$objs=new fileSystem();
			$objs->delFile(_DOMAIN_ROOT_PATH ."/images/gopvon/".$rs->fields("img"));
			$objs->delFile(_DOMAIN_ROOT_PATH ."/images/gopvon/".$rs->fields("img1"));
		}
		
		//xoa trong bang gopvon
		$obj->tableName="sys_gopvon";
		$obj->where="id=$proID";
		$obj->limit="all";
		$sql=$obj->sqlDelete();
	...
	function lockgopvon(){
		...
		$id=getParam("id");		
		$sql="UPDATE sys_gopvon SET ctrl=IF(ctrl=0,1,0) WHERE  id=$id";		
		$db->Execute($sql);
		$sql="SELECT ctrl FROM sys_gopvon WHERE  id=$id";
		$rs=$db->Execute($sql);
	...
	function addPhoto(){
		...
		foreach($_FILES['img_file']['name'] as $name => $value)
		{
			...
			$path_img = _DOMAIN_ROOT_PATH."/images/gopvon/".$name_img;	
			
			move_uploaded_file($source_img, $path_img);
			
			$sql="INSERT INTO sys_gopvon_photo(proid, img, img1) VALUES('$id','$name_img',NOW())";
			$return=$db->Execute($sql); 
		}
		
		include(_DOMAIN_ROOT_PATH . "/classes/msgbox.class.php");		
		if(!$return){
			$ret_page="?m=gopvon&op=photo&id=$id";
			$a=new msgBox(_CANNOT_UPDATE_SUCCESSFU,"OKOnly", "Message", array($ret_page), 1);			
			$a->showMsg();
		}else{
			$ret_page="?m=gopvon&op=photo&id=$id";
	...
	function deletePhoto(){
		...
		$sql="DELETE FROM sys_gopvon_photo WHERE id=$id";		
	...
	function getPhotoID($id){
		global $db;
		$sql="SELECT * FROM sys_gopvon_photo WHERE id=$id";		
		$rs=$db->Execute($sql);
	...
	function addFile(){
		...
		if(!$idFile) $sql="INSERT INTO sys_gopvon_file(proid,file,content,name,date_create,size) VALUES ($id,'$file','$content','$name','$date_create','$size')";		
		else $sql="UPDATE sys_gopvon_file SET file='$file',content='$content',name='$name',date_create='$date_create',size='$size' WHERE id=$idFile";		
		$db->Execute($sql);		

		include(_DOMAIN_ROOT_PATH . "/classes/msgbox.class.php");
		$ret_page="?m=gopvon&op=file&id=$id";
		$a=new msgBox(_UPDATE_SUCCESSFU,"OKOnly", "Message", array($ret_page), 1);			
		$a->showMsg();
	}
	//
	...
	function deleteFile(){
		...
		$sql="DELETE FROM sys_gopvon_file WHERE id=$id";		
		$result=$db->Execute($sql);
		if($result){
			$message=_DELETE_SUCCESSFU;
		}else{
	...
	function getFileID($id){
		global $db;
		$sql="SELECT * FROM sys_gopvon_file WHERE id=$id";		
		$rs=$db->Execute($sql);
	...
	function lockFile(){
		global $db,$lang,$lable;		
		$id=getParam("id");		
		$sql="UPDATE sys_gopvon_file SET ctrl=IF(ctrl=0,1,0) WHERE  id=$id";		
		$db->Execute($sql);
		$sql="SELECT ctrl FROM sys_gopvon_file WHERE  id=$id";
		$rs=$db->Execute($sql);
		echo "<img src=\"images/".$rs->fields("ctrl").".gif\" style=\"cursor:pointer\" />";
	}
	//
	...
	function getGroupAdmin($artID=""){
		...
		$sqlcat="SELECt * FROM sys_gopvon_admin_cat WHERE artID=$artID";	
