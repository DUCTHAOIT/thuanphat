<?php
	function getdoitacList(){
		global $db;
		$sql="SELECT *, DATE_FORMAT(date_create, '".format_date()."') as date FROM user WHERE loai=0 ORDER BY id DESC";		
		$arr=$db->GetAssoc($sql);
		return $arr;
	}
	//	
	 function doitacCreate(){
	...
	//	
	function getdoitacSohuu($username){
		global $db;
