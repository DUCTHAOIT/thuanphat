...
// ✅ Lấy danh sách yêu cầu rút tiền + số dư ví khả dụng hiện tại (kha_dung đã bị trừ sẵn khi tạo yêu cầu)
$sql = "SELECT
            t.*,
            u.name, u.email, u.mobile, u.id as uid,
            IFNULL(w.kha_dung, 0) AS kha_dung
        FROM transactions t
        JOIN user u ON t.user_id = u.id
        ...
        <table class="table table-bordered table-striped">
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($row['mobile']) ?><br><?= htmlspecialchars($row['email']) ?></td>
                    <td><strong style="color:green;"><?= number_format($row['kha_dung'], 0) ?> đ</strong></td>
