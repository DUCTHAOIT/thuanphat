...
$typeLabels = [
    ...
    'card' => 'Thưởng điểm thẻ tiêu dùng',
...
function buildBonusBranch($table, $typeExpr, $typeCheck, $status, $start_date, $end_date, $search) {
    // Chỉ bảng commissions có cột rebuy_id (nguồn gốc giao dịch Rebuy - mục 6 "Giao dịch tái tiêu dùng tự
    // động"); card_point_bonuses/recurring_consumption_bonuses không áp dụng cho Rebuy nên luôn NULL.
    $rebuyIdExpr = $table === 'commissions' ? 't.rebuy_id' : 'NULL';
    $sql = "SELECT t.id, t.order_id, $rebuyIdExpr AS rebuy_id, t.user_id, u.name, u.email, $typeExpr AS type, t.level, t.amount, t.status, t.created_at, t.released_at
...
function buildBonusUnionSql($type, $status, $start_date, $end_date, $search, $commissionTypes) {
    ...
    if ($type === '' || $type === 'card') {
        [$sql, $t, $p] = buildBonusBranch('card_point_bonuses', "'card'", null, $status, $start_date, $end_date, $search);
        $branches[] = $sql;
        $types .= $t;
        $params = array_merge($params, $p);
    }
...
$fundTotals = array_fill_keys(array_keys($fundLabels), 0.0);
...
$stmt->close();
$operatingFundTotal = array_sum($fundTotals);
...
<div class="container-fluid py-4">
    ...
    <h4 class="mt-4 mb-3">Quỹ vận hành (tổng: <?= number_format($operatingFundTotal, 0, ',', '.') ?> đ)</h4>
    <div class="row">
    ...
    <h4 class="mt-4 mb-3">Ví toàn hệ thống (số dư hiện tại)</h4>
    <div class="row">
        <?php
    ...
    <h4 class="mt-4 mb-3">Danh sách hoa hồng / thưởng</h4>
    ...
    <form class="mb-3" method="get">
        ...
        <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
            ...
            <a href="?m=quyhoahong" class="btn btn-secondary">Xóa lọc</a>
        </div>
    </form>
