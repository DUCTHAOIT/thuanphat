<?php
	...
	function mainShow(){
		...
		$smarty->display(_DOMAIN_ROOT_TEMPLATE.'/xuat_su2.tpl','xuat_su2'.$themeName);
	...
	function xuatsuList(){
		...
		if(!$pageID) $pageID=0;
		$limit=100;
		$sql="SELECT * FROM xuat_su WHERE type=1 ORDER BY id DESC";
		$arr=$db->GetAll($sql);
		...
		$smarty->assign('sPage',sPage('?m=user&f=xuatsu2',$numberRecord,$limit,$pageID));
		
		$smarty->display(_DOMAIN_ROOT_TEMPLATE.'/xuat_su_List2.tpl','xuat_su_List_'.$themeName);
	}
	//
	function add()
	{
		...
		if($id)
		{
			$sql="UPDATE xuat_su SET taisan='$taisan', khoiluong='$khoiluong', giadvdt='$giadvdt', type='$type', date='$date', sort='$sort' WHERE (id='$id')";			
			$resurn=$db->Execute($sql);
		}
		else 
		{
			$sql="INSERT INTO xuat_su (taisan,khoiluong,giadvdt,type,date,sort) VALUES ('$taisan','$khoiluong','$giadvdt','$type','$date','$sort')";
			$resurn=$db->Execute($sql);			
		}		
		
		include(_DOMAIN_ROOT_PATH . "/classes/msgbox.class.php");		
		$ret_page="?m=user&f=xuatsu2";
		if(!$resurn)
		{			
