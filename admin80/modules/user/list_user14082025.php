...
/*$sql = "
    SELECT 
        ...
        (SELECT IFNULL(SUM(amount), 0) FROM commissions c WHERE c.user_id = u.id AND c.level = 0) AS hoa_hong_truc_tiep,

        -- Số lượng F1
        (SELECT COUNT(*) FROM user f1 WHERE f1.ref_by = u.id) AS so_luong_f1,

        -- Hoa hồng từ F1 (level 1)
        (SELECT IFNULL(SUM(amount), 0) FROM commissions c WHERE c.user_id = u.id AND c.level = 1) AS hoa_hong_f1,
        ...
        (SELECT IFNULL(SUM(amount), 0) FROM commissions c WHERE c.user_id = u.id AND c.level = 2) AS hoa_hong_f2,

        -- Tổng cộng hoa hồng
        (SELECT IFNULL(SUM(amount), 0) FROM commissions c WHERE c.user_id = u.id) AS tong_hoa_hong
...
<table class="table table-bordered table-striped" border="1" cellpadding="5" cellspacing="0">
    ...
    <?php while ($row = $result->fetch_assoc()): ?>
        <tr>
            ...
            <td><?= number_format($row['hoa_hong_truc_tiep'], 0, ',', '.') ?>đ</td>
