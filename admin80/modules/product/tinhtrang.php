<?php
	...
	function tinhtrangList(){
		...
		$sql="SELECT *,DATE_FORMAT(tinh_trang.date_create, '".format_date()."') as date_create  FROM tinh_trang  ORDER BY id DESC";				
	...
	function add()
	{
		...
		if($id)
		{
			$sql="UPDATE tinh_trang SET name='$name', date_create = '$date_create' WHERE (id='$id')";			
			$resurn=$db->Execute($sql);
		}
		else 
		{
			$sql="INSERT INTO tinh_trang (id,name,date_create) VALUES (NULL,'$name','$date_create')";
			$resurn=$db->Execute($sql);			
		}		
