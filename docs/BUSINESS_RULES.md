...
**Sản phẩm chấp nhận nguồn nào** (bổ sung 2026-07-11, cập nhật 2026-07-13 - thêm Tích lũy tiêu dùng): mỗi
sản phẩm (`sys_product`) có 4 cờ riêng `accept_card_payment` / `accept_tich_luy_payment` /
`accept_tieu_dung_payment` / `accept_kha_dung_payment` (mặc định = 1, admin tự tắt trong form Thêm/Sửa sản
phẩm `?m=product` nếu muốn chặn). Chuyển khoản/tiền mặt **luôn luôn** được chấp nhận, không cấu hình tắt
được (phương án dự phòng cuối cùng đảm bảo khách luôn đặt được đơn). Giỏ hàng nhiều sản phẩm: 1 nguồn chỉ
**hiện cho khách chọn** nếu **TẤT CẢ** sản phẩm trong giỏ đều chấp nhận nguồn đó (lấy giao — intersection,
không tính riêng theo từng dòng sản phẩm). Server luôn tính lại giao này khi tạo đơn (không tin lựa chọn
checkbox gửi từ client) — xem `getAcceptedPaymentSources()` trong `modules/basket/action.php`, dùng chung
cho cả trang giỏ hàng (ẩn/hiện checkbox) và `modules/basket/order.php` (chặn nguồn không được phép trước khi
trừ tiền).
...
2. Phần còn lại → trừ vào ví Tích lũy tiêu dùng (bổ sung 2026-07-13, mục 2, mục 6).
3. Phần còn lại → trừ vào ví tiêu dùng.
4. Nếu ví tiêu dùng hết điểm → phần còn thiếu trừ tiếp vào ví khả dụng.
5. Nếu cả 4 nguồn trên vẫn chưa đủ (hoặc khách không tích chọn nguồn nào) → phần còn lại thanh toán bằng
...
**Hoa hồng tính trên "quỹ chia hoa hồng"** = tổng "hoa hồng sản phẩm" từng dòng trừ đi điểm thẻ tiêu dùng
...
- **Quỹ chia hoa hồng** = tổng ("hoa hồng sản phẩm" × số lượng) của từng dòng sản phẩm trong đơn, **trừ đi**
  số tiền đã thanh toán bằng điểm thẻ tiêu dùng **và** Tích lũy tiêu dùng cho đơn đó
  (`order_payments.card_amount` + `order_payments.tich_luy_amount`, cập nhật 2026-07-13 — xem mục "Quy tắc
  thanh toán đơn hàng" phía trên).
- **Quỹ công ty** = giá trị đơn hàng − quỹ chia hoa hồng (tính trên số tiền **trước** khi trừ điểm thẻ/Tích
  lũy tiêu dùng — 2 khoản này chỉ làm giảm quỹ chia hoa hồng, không ảnh hưởng quỹ công ty). Không tự động
  40% cố định nữa, tỉ lệ % thực tế tùy admin đặt "hoa hồng sản phẩm" bao nhiêu cho từng sản phẩm. Quỹ này
  chỉ mang tính khái niệm, không lưu/track ở đâu trong hệ thống (không có nghiệp vụ nào dùng tới).
- Ví dụ: đơn 1,000,000đ, tổng "hoa hồng sản phẩm" các dòng = 600,000đ, dùng 100,000đ điểm thẻ + 50,000đ
  Tích lũy tiêu dùng thanh toán → quỹ chia hoa hồng = 600,000 − 100,000 − 50,000 = 450,000đ; quỹ công ty vẫn
  = 1,000,000 − 600,000 = 400,000đ (không bị trừ 2 khoản này).
...
(cập nhật 2026-07-15: chỉ còn nhận **1 nguồn** — trước đó còn nhận thêm 30% của thưởng tiêu dùng tuần hoàn, nay 30% đó tách sang "Quỹ tiêu dùng tuần hoàn công ty" riêng, xem bên dưới.)

Quỹ vận hành do admin quản lý, nhận tiền từ:
...
## Quỹ tiêu dùng tuần hoàn công ty

(bổ sung 2026-07-15, tách riêng khỏi quỹ vận hành ở trên)

Nhận **30%** còn lại của khoản thưởng tiêu dùng tuần hoàn (mục 6) — trước đó khoản này đổ chung vào quỹ vận hành, nay tách thành quỹ riêng, chia đều **3 phần bằng nhau** (mỗi phần = 10% quỹ chia hoa hồng gốc, cộng lại đúng 30% như cũ):

| Ví | Tỉ lệ (trên khoản tiền vào quỹ này) | fund_code |
|---|---|---|
| Chi phí nền tảng (nhân sự, IT) | 1/3 (~10% quỹ chia hoa hồng gốc) | `cp_nen_tang` |
| Ban điều hành & Leader | 1/3 (~10% quỹ chia hoa hồng gốc) | `bdh_leader` |
| Quỹ dự phòng (thẻ tiêu dùng) | 1/3 (~10% quỹ chia hoa hồng gốc) | `du_phong_the` |

Quỹ dự phòng ở đây **tách biệt hoàn toàn**, không gộp số dư với "Quỹ dự phòng" (`du_phong`) của quỹ vận hành ở trên.

Chỉ ghi log cộng tiền (giống quỹ vận hành), không có nghiệp vụ chi tiêu/trừ ví.

(cập nhật 2026-07-15: đã code — `creditCompanyCardFund()` trong `admin80/include/order_commission.php`, ghi vào bảng `admin_fund_transactions` (mục 11.2, dùng chung `fund_code` với quỹ vận hành nhưng 3 giá trị riêng), hiển thị tại trang admin `?m=quyhoahong`. Xem `database/migration_2026-07-15_company_card_fund.sql`.)
...
- Hoa hồng cây lấp tầng + thẻ tiêu dùng tuần hoàn + Tích lũy tiêu dùng + thưởng danh hiệu (gọi
...
## Quy tắc pending / release (chỉ áp dụng cho "thưởng hệ thống" — hoa hồng cây lấp tầng, thẻ tiêu dùng tuần hoàn, Tích lũy tiêu dùng, thưởng danh hiệu; **không áp dụng cho hoa hồng sơ đồ trực tiếp**, xem mục 4)
...
toàn bộ hoa hồng cây lấp tầng + 3 khoản thưởng (thẻ tiêu dùng tuần hoàn, Tích lũy tiêu dùng, danh hiệu) ngay
...
(cập nhật 2026-07-13: đổi tên từ "thưởng điểm thẻ tiêu dùng" — vẫn giữ nguyên tỉ lệ trích 10% và thời điểm
phát sinh, chỉ đổi đối tượng nhận và nơi lưu. Khác hẳn "**điểm thẻ tiêu dùng**" ở mục 2/3 —
`consumption_cards.balance`, 5,000,000đ cộng khi kích hoạt gói 5tr, thanh toán ưu tiên 1 — 2 khoản này độc
lập, không liên quan nhau.)

Tính trên cây lấp tầng (hay điều tầng) — Khi thành viên được xếp vào cây mới chia (hoặc tính bù, xem "Nguồn
quỹ hoa hồng + thưởng cây lấp tầng" phía trên).

Trích **10%** quỹ chia hoa hồng (ví dụ quỹ 3,000,000đ → trích 300,000đ) cho khoản này, chia **đều** cho
**toàn bộ thành viên `business_active = 1` trong hệ thống**, **trừ chính người vừa được xếp vào cây** (nguồn
phát sinh quỹ này) — **không** còn chia theo 8 tầng cây lấp tầng như trước (khác hẳn Hoa hồng cây lấp tầng
và Thưởng tiêu dùng tuần hoàn ở trên, 2 khoản đó vẫn chia theo tầng). Nếu không có ai đủ điều kiện nhận
(toàn hệ thống chỉ có 1 mình người vừa kích hoạt là `business_active`) thì bỏ qua, không dồn cho lần phát
sinh khác.
...
**Dùng thanh toán đơn hàng** — ưu tiên 2, ngay sau điểm thẻ tiêu dùng (mục 3). Khi dùng thanh toán, số tiền
đã dùng (`order_payments.tich_luy_amount`) **trừ khỏi quỹ chia hoa hồng** của đơn đó, giống điểm thẻ tiêu
dùng (mục 3).

Sản phẩm chấp nhận thanh toán bằng Tích lũy tiêu dùng hay không cấu hình qua cờ `sys_product.accept_tich_luy_payment`
(mục 3, mục 11), admin tự tắt trong form Thêm/Sửa sản phẩm.

Giao dịch Rebuy (tái tiêu dùng tự động) **không** sinh thêm Tích lũy tiêu dùng, giống thưởng tiêu dùng tuần
hoàn và thưởng điểm thẻ tiêu dùng cũ (xem "Giao dịch tái tiêu dùng tự động" bên dưới).

Code: `generateAccumulatedConsumptionBonus()` / `creditAccumulatedConsumptionBonus()` /
`releasePendingAccumulatedConsumptionBonuses()` trong `admin80/include/order_commission.php`, bảng
`accumulated_consumption_bonuses` (mục 11). Cơ chế cũ (bảng `card_point_bonuses`, chia theo 8 tầng, cộng vào
`consumption_cards.balance`) giữ nguyên dữ liệu lịch sử, không còn được ghi mới.
...
Giao dịch Rebuy **không** sinh thêm thưởng tiêu dùng tuần hoàn và Tích lũy tiêu dùng (2 khoản này chỉ áp dụng cho đơn kích hoạt gói 5tr, xem phần "thưởng tiêu dùng tuần hoàn" / "Tích lũy tiêu dùng" ở trên).
...
- Số tiền rút **tối thiểu 100.000đ/lần** (bổ sung 2026-07-15) — validate cả client (modal rút tiền,
  `modules/user/dashboard.php`) và server (`modules/user/xu_ly_rut_tien.php`), có ghi chú cho user ngay trong
  form nhập số tiền.
...
| `accept_card_payment` / `accept_tich_luy_payment` / `accept_tieu_dung_payment` / `accept_kha_dung_payment` | TINYINT(1) DEFAULT 1 | (đã áp dụng — `database/migration_2026-07-11_product_payment_sources.sql` + `database/migration_2026-07-13_accumulated_consumption_wallet.sql` cho `accept_tich_luy_payment`) Sản phẩm có chấp nhận thanh toán bằng điểm thẻ tiêu dùng / Tích lũy tiêu dùng / ví tiêu dùng / ví khả dụng hay không, admin tự tắt trong form Thêm/Sửa sản phẩm. Mặc định 1 (chấp nhận) cho mọi sản phẩm. Chuyển khoản/tiền mặt luôn được chấp nhận, không có cột riêng (mục 3). |
...
| `id` | INT PK AI | |
| `user_id` | INT | |
| `wallet_type` | ENUM('tong','kha_dung','tieu_dung','tai_tieu_dung','thue_phi','tich_luy_tieu_dung') | Ví nào bị tác động (`tich_luy_tieu_dung` bổ sung 2026-07-13). |
| `direction` | ENUM('credit','debit') | Cộng hay trừ. |
| `amount` | DECIMAL(15,2) | |
| `balance_after` | DECIMAL(15,2) | Số dư sau giao dịch, phục vụ đối soát/audit. |
| `ref_type` | ENUM('order','commission','withdraw','rebuy','refund','rank_bonus','card_bonus','admin_adjust','accumulated_consumption') | Nguồn gốc phát sinh giao dịch (`accumulated_consumption` bổ sung 2026-07-13 — release Tích lũy tiêu dùng). |
...
Khác với `commissions`/`accumulated_consumption_bonuses`: khi released, cộng thẳng vào `user_wallets.tieu_dung` (không qua quy tắc chia ví tổng 60/20/10/10 ở mục 2).

### `card_point_bonuses` — thưởng điểm thẻ tiêu dùng (cơ chế cũ, ĐÃ NGỪNG DÙNG từ 2026-07-13)

Bảng cũ của cơ chế trước khi đổi tên/đổi cách chia thành "Tích lũy tiêu dùng" (mục 6). Giữ nguyên dữ liệu lịch
sử để tra cứu, code không còn ghi dòng mới vào bảng này — xem `accumulated_consumption_bonuses` bên dưới.
...
| `order_id` | INT | |
| `user_id` | INT | |
| `level` | INT | Tầng (1-8) trong cây lấp tầng — cơ chế mới không còn khái niệm tầng cho khoản này. |
| `amount` | DECIMAL(15,2) | |
| `status` | ENUM('pending','released') DEFAULT 'released' | |
| `released_at` | DATETIME NULL | |
| `created_at` | DATETIME | |

### `accumulated_consumption_bonuses` — Tích lũy tiêu dùng (mục 6, bổ sung 2026-07-13 — `database/migration_2026-07-13_accumulated_consumption_wallet.sql`)

| Cột | Kiểu | Chú thích |
|---|---|---|
| `id` | INT PK AI | |
| `order_id` | INT | Đơn kích hoạt gói 5tr làm nền tính quỹ. |
| `user_id` | INT | Người nhận — 1 trong toàn bộ thành viên `business_active = 1`, không nhất thiết có vị trí trong cây lấp tầng liên quan tới người vừa kích hoạt (khác `recurring_consumption_bonuses`/`card_point_bonuses` cũ - không có cột `level` vì không còn chia theo tầng cây). |
| `amount` | DECIMAL(15,2) | |
| `status` | ENUM('pending','released') DEFAULT 'pending' | Theo `commission_active` của người nhận (mục 5). |
| `released_at` | DATETIME NULL | |
| `created_at` | DATETIME DEFAULT CURRENT_TIMESTAMP | |

Khi released, cộng thẳng vào `user_wallets.tich_luy_tieu_dung` (ví riêng, không qua quy tắc chia ví tổng 60/20/10/10 ở mục 2, không cộng chung với `consumption_cards.balance`).
...
### `admin_fund_transactions` — 5 quỹ con của quỹ vận hành + 3 ví của Quỹ tiêu dùng tuần hoàn công ty

Các quỹ này chỉ hiển thị trên admin, không thao tác trực tiếp (mục 3 — Quỹ vận hành / Quỹ tiêu dùng tuần hoàn công ty). Dùng chung 1 bảng cho cả 2 nhóm quỹ, phân biệt bằng `fund_code` (cập nhật 2026-07-15 — thêm 3 giá trị mới, xem `database/migration_2026-07-15_company_card_fund.sql`).

| Cột | Kiểu | Chú thích |
|---|---|---|
| `id` | INT PK AI | |
| `fund_code` | ENUM('thi_truong_leader','van_phong','dao_tao','it_van_hanh','du_phong','cp_nen_tang','bdh_leader','du_phong_the') | 5 giá trị đầu = quỹ vận hành (mục 3); 3 giá trị sau (bổ sung 2026-07-15) = Quỹ tiêu dùng tuần hoàn công ty (mục 3) — `du_phong_the` tách biệt hoàn toàn với `du_phong`. |
| `source` | ENUM('direct_commission','card_bonus','rebuy') | direct_commission = 10% từ quỹ chia hoa hồng mỗi đơn (mục 3), luôn đi kèm 1 trong 5 `fund_code` của quỹ vận hành; card_bonus = 30% còn lại của thưởng tiêu dùng tuần hoàn (mục 6), từ 2026-07-15 luôn đi kèm 1 trong 3 `fund_code` của Quỹ tiêu dùng tuần hoàn công ty (trước đó đi kèm 5 fund_code của quỹ vận hành — dữ liệu lịch sử giữ nguyên); rebuy = phần còn lại của quỹ hoa hồng Rebuy sau khi chia 3 khoản (mục 6, cập nhật 2026-07-11 — đã áp dụng), luôn đi kèm quỹ vận hành. Mỗi nguồn khi vào quỹ vận hành chia đều 20%/quỹ con (5 quỹ); khi vào Quỹ tiêu dùng tuần hoàn công ty chia đều 1/3 (3 ví). |
...
hồng, mục 3. Cập nhật 2026-07-13: thêm `tich_luy_amount`, cùng được `calculateCommissionFund()` trừ khỏi quỹ
chia hoa hồng giống `card_amount`.)
...
| `tich_luy_amount` | DECIMAL(15,2) DEFAULT 0 | (bổ sung 2026-07-13) Số tiền trừ từ ví Tích lũy tiêu dùng, ưu tiên 2 (mục 3). Được `calculateCommissionFund()` đọc lại để trừ khỏi quỹ chia hoa hồng, giống `card_amount`. |
| `tieu_dung_amount` | DECIMAL(15,2) DEFAULT 0 | Trừ từ ví tiêu dùng. Chỉ dùng để hoàn tiền khi đơn bị từ chối (`refundOrderPaymentIfAny()`) — **không** trừ khỏi quỹ chia hoa hồng (xác nhận 2026-07-11: quỹ chia hoa hồng chỉ trừ `card_amount` + `tich_luy_amount`). |
...
| `commission_base_amount` | DECIMAL(15,2) | Chỉ mang tính ghi nhận/audit = giá trị đơn hàng − `card_amount`. **Không** phải nền tính hoa hồng thực tế (nền tính hoa hồng dùng `calculateCommissionFund()` — tổng "hoa hồng sản phẩm" từng dòng trừ `card_amount` + `tich_luy_amount`, xem mục 3). |
