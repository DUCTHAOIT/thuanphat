<?php 
	include_once("header.php");	
		...
		$sqlupdown="SELECT * FROM  `sys_hieuqua` WHERE (ctrl&1=1) ORDER BY date_create DESC LIMIT 0 , 2";	
...
<div class="topicContent" style="border-bottom:1px solid #CCCCCC"><h1>Biểu đồ hiệu quả đầu tư</h1></div>
                ...
                <div>
                  <div class="box_hieuquadautu">
					...
					for($i=0;$i<count($arrupdown); $i++)
					{
						...
						if($tanggiam>0){ echo "<span style=\"font-size:16px;\">U ".format_number2($giatri1)."&nbsp;&nbsp;</span><span style=\"color:green\">".$tanggiam."%</span><span class=\"fa fa-arrow-up\" style=\"color:green\" aria-hidden=\"true\"></span>";}else{echo "<span style=\"font-size:16px;\">U ".format_number2($giatri1)."&nbsp;&nbsp;</span><span style=\"color:red\">".$tanggiam."%</span><span class=\"fa fa-arrow-down\" style=\"color:red\" aria-hidden=\"true\"></span>";}
					?>
                    </div>
        ...
        var data = [
		 ...
		 <?php
			for($i=0;$i<count($arr); $i++)
			{
				...
				$tangtruongvnindex=($arr[$i]["giatri3"]-718.45)/718.45*100;
