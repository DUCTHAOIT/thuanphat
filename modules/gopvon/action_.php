<?php		
		...
		if($rs->RecordCount()){
           ...
           <table width="100%" border="0" cellspacing="0" cellpadding="0">
             ...
             <?php
				...
				?>	             
                <td style="width:33%; padding:10px;" valign="top" align="left">
                        <div style="border:1px solid #CCCCCC; background-color:#FFFFFF; width:224px">
                             <a  href="<?php echo _DOMAIN_ROOT_URL."/".$rs->fields("htaccess")?>" class="content" ><img src="<?php echo _DOMAIN_ROOT_URL?>/image.php?width=200&image=<?php echo _DOMAIN_ROOT_URL?>/images/function/<?php echo $rs->fields("img1");?>" border="0" hspace="2" vspace="2" width="220" height="140px" /></a>
                        </div>													

                    <div style="padding-top:5px">
                        <a href="<?php echo _DOMAIN_ROOT_URL."/".$rs->fields("htaccess")?>" class="title" ><?php echo $rs->fields("name");?></a>
                    </div>
                    <div style="padding-top:5px; padding-bottom:5px; text-align:justify"><?php echo strstrim($rs->fields("des"),45);?></div> 
                    <div class="contentChitiet"> <a href="<?php echo _DOMAIN_ROOT_URL."/".$rs->fields("htaccess")?>" class="content" >Chi tiết</a></div>
                </td>	
               <?php 
					$j++;
					$rs->MoveNext();						
	...
	//
	function getProductListFaq($proID){
		global $db;		
		$sql="SELECT * FROM sys_commentproduct WHERE (ctrl&1=1) AND (proid=$proID)";		
		$arr=$db->GetAssoc($sql);
		return $arr;
	}	
?>