<?php
function usergioithieu(){
	global $db,$lang,$lable;	
	$username=getSession("username");
	if(!$username) return;
	
	$sql="SELECT * FROM xuat_su WHERE type=0 ORDER BY id DESC LIMIT 0,1";
...
<?php
	...
	$giatrimua=0;
	$tonglailogioithieu=0;
	$tonggiatrimua=0;
	
	//sys_userorder.trangthai=0 lay cac hop dong chua tat toan
	$sql="SELECT sys_userorder.*, DATE_FORMAT(sys_userorder.date_create, '".format_date()."') as date_create";
	$sql.=" FROM user, sys_userorder";
	$sql.=" WHERE (user.email = '".$rs->fields("username")."') AND (user.id = sys_userorder.userid) AND (sys_userorder.ctrl&1=1) AND (sys_userorder.trangthai=0) AND ((sys_userorder.model>0) OR (sys_userorder.product_in>0)) AND (sys_userorder.loai=0)";
