...
function lichsugiaodich456(){
	...
	$sql="SELECT sys_userorder.*, DATE_FORMAT(sys_userorder.date_create, '".format_date()."') as date_create, TO_DAYS(sys_userorder.date_create) as today";
	$sql.=" FROM user, sys_userorder";
	$sql.=" WHERE (user.id = sys_userorder.userid) AND (sys_userorder.ctrl=0) AND (sys_userorder.tinh_trang=1) AND (sys_userorder.trangthai=1) AND (sys_userorder.loai=1)";
	if($monthlichsu){
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <select name="year" onchange="location = '<?php echo _DOMAIN_ROOT_URL."/user_buy_sell456/"; ?><?php if($monthlichsu){ echo "&monthlichsu=".$monthlichsu;} ?>&yearlichsu='+ this.options[this.selectedIndex].value;" style="font-size:12px" >
    ...
    <td align="right" nowrap="nowrap">			
    <select name="month" onchange="location = '<?php echo _DOMAIN_ROOT_URL."/user_buy_sell456/"; ?><?php if($yearlichsu){ echo "&yearlichsu=".$yearlichsu;} ?>&monthlichsu='+ this.options[this.selectedIndex].value;" style="font-size:12px" >
