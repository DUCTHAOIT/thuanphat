<?php
function userxacthucbanpc(){
	global $db,$lang,$lable;	
	$username=getSession("username");
	if(!$username) return;
	
	$sql="SELECT *,TO_DAYS(xuat_su.date_create) as today, TO_DAYS(CURDATE()) as homnay FROM xuat_su WHERE sort=1 AND type=0 ORDER BY id DESC LIMIT 0,1";
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" class="table">
  <tr class="tr" style="background-color:#CCCCCC">
    ...
    <td>Ngày mua trong HĐ/<br /> Ngày đặt lệnh</td>
    <td>Số lượng ĐVĐT</td>
...
?>
  ...
  <tr <?php if($j%2){?> bgcolor="#F4F4F4"<?php }?>>
    <td style="text-align: center; vertical-align: middle;"><?php echo $rshdban->fields("name") ?></td>
    <td style="text-align: center; vertical-align: middle;"><?php echo $rshdban->fields("date_create") ?><br /><?php echo $rs->fields("date_create") ?></td>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php echo number_format($rshdban->fields("model")+$rshdban->fields("product_in"), 0, '.', ',') ?></td>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php echo number_format($rshdban->fields("price"), 0, '.', ',') ?></td>
    ...
    <?php }?>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php echo number_format($rs->fields("model"), 0, '.', ',') ?></td>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot){ echo number_format($giadvdtchot, 0, '.', ',')."";}else{?> <font color="#FF0000" style="font-size:16px">*</font><?php }?></td>
    ...
    <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot>0){ echo number_format($lailo, 0, '.', ',')."";}else{ echo "#";} ?></td>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot>0){ echo number_format($phihoptac, 0, '.', ',')."";}else{ echo "#";} ?></td>
   
    <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot>0){ echo number_format($phiruttruochan, 0, '.', ',')."";}else{ echo "#";}?></td>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot>0){ echo number_format($hoahongck, 0, '.', ',')."";}else{ echo "#";} ?></td>
    <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot>0){ echo number_format($tncn, 0, '.', ',')."";}else{ echo "#";} ?></td>
     <td align="right" style="text-align: center; vertical-align: middle;"><?php if($giadvdtchot>0){ echo number_format($thucnhan, 0, '.', ',')."";}else{ echo "#";} ?></td>
  	<?php if($rs->fields("trangthai")==1){ echo "<td nowrap=\"nowrap\" align=\"center\" class=\"title\"  style=\"color:#0000FF; text-align: center; vertical-align: middle;\">Đã bán</td>";}?>
     <?php if($rs->fields("trangthai")==2){ echo "<td nowrap=\"nowrap\" align=\"center\" class=\"title\" style=\"color:#FF0000; text-align: center; vertical-align: middle;\">Bị hủy bỏ</td>";}?>
     <?php if($rs->fields("trangthai")==0){ ?>
	<?php if($rs->fields("price")){echo "<td nowrap=\"nowrap\" align=\"center\"  style=\"text-align: center; vertical-align: middle;\">Đã xác nhận</td>";}else{?>
    <td nowrap="nowrap" align="center" style="text-align: center; vertical-align: middle;"  >
...
?>
<tr>
	<td colspan="15"><font color="#FF0000" style="font-size:16px">*</font> Giá ĐVĐT rút vốn là giá trị đóng cửa (sau 15h30) ngày thứ 5 hàng tuần</td>
</tr>
</table>
</div>
<?php 
