...
function userxacthucmua456(){
	...
	$sql.=" WHERE (user.username = '$username') AND (user.id = sys_userorder.userid) AND (sys_userorder.tinh_trang=0) AND (sys_userorder.loai=1) AND (sys_userorder.ctrl=0)";
...
?>
 <tr class="tr" style="background-color:#CCCCCC">
  ...
  <tr <?php if($j%2){?> bgcolor="#F4F4F4"<?php }?>>
    <td style="text-align: center; vertical-align: middle; font-size:13px"><?php echo $rs->fields("name") ?></td>
    <td align="right" style="text-align: center; vertical-align: middle; font-size:13px"><?php echo $rs->fields("date_create") ?></td>
    <td align="right" style="text-align: center; vertical-align: middle; font-size:13px"><?php echo number_format($rs->fields("price_old"), 0, '.', ',') ?></td>
    <td align="right" style="text-align: center; vertical-align: middle; font-size:13px"><?php echo number_format($rs->fields("price"), 0, '.', ',') ?></td>
    <td align="right" style="text-align: center; vertical-align: middle; font-size:13px"><?php echo number_format($rs->fields("model"), 0, '.', ',') ?></td>
    <td align="center" style="text-align: center; vertical-align: middle; font-size:13px"><?php if($rs->fields("ctrl")==0){ echo "<font  class=\"title\" style=\"color:#FF0000\">Đang thực hiện</font>";}else{ echo "<font  class=\"title\" style=\"color:#0000FF\">Đã hoàn thành</font>";} ?></td>
  </tr>
	 <tr>
        <td  colspan="6"><p>Quý khách chuyển khoản cho TSI theo thông tin dưới để hoàn tất việc đặt mua</p></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="4"><p id="p4" style="display:none"><?php echo $MemberName;?> <?php echo $rs->fields("name") ?> nop tien vao tai khoan 8883681 cua CTCP Dau tu phat trien Thach Sanh</p><p><a onclick="copyToClipboard('p4')" title="Click để copy" style="cursor:pointer;"><?php echo $MemberName;?> <?php echo $rs->fields("name"); ?> nop tien vao tai khoan 8883681 cua CTCP Dau tu phat trien Thach Sanh <i class="fa fa-files-o" aria-hidden="true" style="font-size:13px" ></i></a></p></td>
      </tr>
	<tr>
