<?php
	...
	function mainShow(){
		...
		$smarty->registerPlugin("function","postList", "postList");
	...
	function frmpost(){
		...
		if(!$username){
			...
			$smarty->assign('id',$id);		
			$smarty->assign('date_create',date('Y-m-d', strtotime("+90 days")));		
	...
	function postPhoto(){
		...
		$smarty->registerPlugin("function","postListPhoto", "postListPhoto");
	...
	{		
		...
		//$smarty->register_function("sobinhluan", "sobinhluan");
		//$smarty->register_function("soluu", "soluu");
		
		$smarty->assign('quality_certificate',$lable->_("quality certificate"));
		//$smarty->registerPlugin("function","product_list_user", "product_list_user");
		$smarty->display(_DOMAIN_ROOT_TEMPLATE.'/userBuySell.tpl','userBuySell_');
	...
	function product_list_user()
	{
		...
		if($MemberID){
			$arr=get_product_list_user_off($MemberID);
		}else{
